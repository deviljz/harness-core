...
> **状态累积自检（涉及重试 / 失败恢复 / 乐观渲染列表时必须）**：重试类必须测「失败 → 重试 → 再失败 → 再重试，连续 N 轮断言同一项状态不累积（数量不翻倍）」。单次重试看不出（会被成功清除掩盖）。典型坑（2026-06 选图打卡）：重试时 `resetFailed`(清红但项还在列表) + 上传路径又 `addPaths` 一份 → 缩略图 1→2→4 翻倍。涉及"重试 / retry / 失败恢复"但 §6 无累积守卫关键词 → validate 报 warning。

> **异步慢依赖中间态自检（涉及上传进度 / loading / 乐观渲染时必须）**：必须注入「慢依赖」(onProgress 喂到 1.0 后 Future 故意延迟 resolve 的 fake service)，断言中间态——**进度 100% ≠ 完成**（字节发完到服务器响应+刷新之间有空窗）。用秒传 / 小图验证会把空窗压成 0ms，**等于删掉唯一能暴露 bug 的条件**。典型坑（2026-06 选图打卡）：缩略图停在「100%」+「+」按钮转圈被误读为另一张图在传。涉及"进度 / loading / 乐观渲染"但 §6 无慢依赖中间态关键词 → validate 报 warning。
...
## 9. 数据采集 vs 渲染层边界（涉及"展示某项数据"时必填，否则写 N/A）

> 经验教训：很多"渲染 bug"根因在采集端**默认关闭 / 不采**。如 Periscope `EnableAssetSnapshots=false` 导致整页空，反复试渲染 regex 才意识到是采集没开。spec 阶段就要把这层边界**显式化**。

### 每个要展示的数据点列一行

| 字段 / 图表 | 数据来源 | 采集器 / 上游 API | 默认开关 | 真机 / Editor 限制 | 数据缺失时 UI 怎么显示 |
|---|---|---|---|---|---|
| 例：chartMemTotal | COUNTERS[IDX_TOTU] | UnityMemoryCapture | 默认 ON | 真机/Editor 都采 | 显示「无数据」灰条 |
| 例：用户作文 OCR 文字 | OCR 接口 | gemini-3-flash | 配额内 ON，超配额 OFF | — | 显示「请稍后重试」 |

### 默认 OFF 的字段必须在 §1 Objective 「成功标准」里**显式说明**：用户怎么开/谁来开

避免"以为只是 UI 没写好，实际是采集开关默认关闭"的反复定位。

---

## 10. 参考实现 / Benchmark（对标某个已有实现时必填，否则写 N/A）

> 经验教训：对标类工程（如 Periscope vs UWA）开发 30 轮后才发现 31 子页 vs 15 tab 的覆盖度差距。spec 阶段不强制"列参考实现 + 覆盖差距"，工程师视角"已实现 8 模块" ≠ 用户视角"功能只有一半"。
...
- 名称 / 链接：
- 版本 / 日期（重要：参考实现也在变）：
- 抓取方式：URL（chrome MCP 自动） / 本地 HTML / 截图 / 文档描述

### 覆盖度差距清单

| 大类 | 参考实现有 | 当前实现 | 状态 ✓/🟡/❌ | 本次是否覆盖 |
|---|---|---|---|---|
| 例：性能简报 | 14 个 KPI 卡 | 已实现 | ✓ | — |
| 例：GPU 指标汇总 | 独立子页 | 无 | ❌ | 不做（理由：暂无 GPU API） |

### Boundaries 的衔接

§7 Boundaries「绝不做」段必须列出**确认不做的覆盖差距项**（含理由），避免"漏了"被混淆为"不做"。
